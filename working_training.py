#!/usr/bin/env python3
"""
Working CNN Training for Glaucoma Classification
This script properly handles the dataset structure and trains a CNN model
"""
...
def setup_logging():
    ...
    log_filename = f"logs/working_training_{timestamp}.log"
...
class WorkingDataGenerator(keras.utils.Sequence):
    """
    Working data generator for grayscale images
    """
    ...
    def _load_batch(self, indices):
        """Load batch from HDF5 with proper handling"""
        # Sort indices for h5py compatibility
        sorted_indices = np.sort(indices)
        reverse_map = np.argsort(np.argsort(indices))
        
        with h5py.File(self.h5_file_path, 'r') as f:
            batch_labels = f['labels'][sorted_indices].astype(np.float32)
            ...
            # Simple normalization to [0, 1] without min/max issues
...
def create_working_cnn_model(num_classes=5, dropout_rate=0.5):
    """
    Create a working CNN model for grayscale images
    """
    ...
    logger.info(f"Working CNN model created with {model.count_params():,} parameters")
    return model
...
def train_fold(fold, train_indices, val_indices, h5_file_path, config):
    ...
    train_generator = WorkingDataGenerator(
    ...
    val_generator = WorkingDataGenerator(
        h5_file_path=h5_file_path,
        batch_size=config['batch_size'],
        shuffle=False,
        augment=False,
    ...
    # Create model
    model = create_working_cnn_model(
    ...
    # Create callbacks
    model_save_path = f"models/working_cnn_fold_{fold + 1}.h5"
...
def create_visualizations(results, config):
    ...
    plt.tight_layout()
    plt.savefig('visualizations/working_cnn_results.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    logger.info("Visualizations saved to visualizations/working_cnn_results.png")
...
def main():
    ...
    """
    logger.info("Starting Working CNN Glaucoma Classification Training")
    ...
    if all_results:
        ...
        # Save results
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_file = f"logs/working_cnn_results_{timestamp}.json"
        ...
        best_model_path = f"models/working_cnn_fold_{best_fold['fold']}.h5"
        final_model_path = "models/working_cnn_best_model.h5"
